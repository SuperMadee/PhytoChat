...
train_parquet_file = "train_df.parquet"  # File path for the training dataset
test_parquet_file = "test_df.parquet"  # File path for the test dataset

train_dataset = load_dataset("parquet", data_files=train_parquet_file)  # Loading the training dataset using the load_dataset function
test_dataset = load_dataset("parquet", data_files=test_parquet_file)  # Loading the test dataset using the load_dataset function
# Create a DatasetDict
dataset = DatasetDict({
    'train': train_dataset['train'], # Creating a DatasetDict with the training dataset
    'test': test_dataset['train']  # Adding the test dataset to the DatasetDict
})

dataset = load_dataset("parquet", data_files={'train': train_parquet_file, 'test': test_parquet_file})  # Loading the datasets using the load_dataset function
...
# Generate text using the base model
with torch.no_grad():
    print(tokenizer.decode(merged_model.generate(**model_input, max_new_tokens=256, pad_token_id=2, repetition_penalty=1)[0], skip_special_tokens=True))
